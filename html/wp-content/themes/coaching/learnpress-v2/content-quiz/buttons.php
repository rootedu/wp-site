...
if ( $user->has( 'finished-course', $course->id ) ) {
	//learn_press_display_message( __( 'You have already finished course but have not started this quiz.', 'coaching' ) );
	return;
}
...
<form class="quiz-buttons" method="post">
	...
	<?php if ( $user->has( 'quiz-status', array( 'started' ), $quiz->id ) ): ?>
		<button class="button-prev-question"><?php esc_html_e( 'Previous', 'coaching' ); ?></button>
		<button class="button-next-question"><?php esc_html_e( 'Next', 'coaching' ); ?></button>
		<?php if ( !$user->has_checked_answer( $question->id, $quiz->id, $course->id ) ): ?>
			<button
				...
				<?php esc_html_e( 'Check', 'coaching' ); ?>
			</button>
			<button class="button-hint" data-security="<?php esc_attr_e( wp_create_nonce( 'get-question-hint-' . $user->id . '-' . $course->id . '-' . $quiz->id ) ); ?>"><?php esc_html_e( 'Hint', 'coaching' ); ?></button>
		<?php endif; ?>
	<?php endif; ?>
	...
	<?php if ( $user->has( 'quiz-status', 'completed', $quiz->id ) ): ?>
		...
		<?php if ( $remain = $user->can( 'retake-quiz', $quiz->id ) ): ?>
			<button
				...
				<?php echo esc_html( sprintf( '%s (+%d)', __( 'Retake', 'coaching' ), $remain ) ); ?>
			</button>
		<?php endif; ?>
	...
	<?php elseif ( $user->can_do_quiz( $quiz->id, $course->id ) ): ?>
		<button
			...
			<?php esc_html_e( 'Start Quiz', 'coaching' ); ?>
		</button>
	<?php endif; ?>
	...
	<?php if ( in_array( $status, array( 'started' ) ) ): ?>
		<button
			...
			<?php esc_html_e( 'Finish Quiz', 'coaching' ); ?>
		</button>
	<?php endif; ?>
