...
<?php else: ?>
	...
	<table class="lp-pmpro-membership-list">
		...
		<tr class="item-row">
			<td class="list-main item-td item-desc"><?php esc_html_e( 'Number of courses', 'coaching' ); ?></td>
			<?php
			foreach ( $levels as $index => $level ) {
				$the_query = lp_pmpro_query_course_by_level( $level->id );
		...
		<tr class="item-row">
			<td class="list-main item-td item-desc"><?php esc_html_e( 'Time', 'coaching' ); ?></td>
			<?php foreach ( $levels as $index => $level ) {
				$expired = '';
				if ( $level->expiration_number ) {
					$expired = sprintf( __( "%d %s", "coaching" ), $level->expiration_number, pmpro_translate_billing_period( $level->expiration_period, $level->expiration_number ) );
				}
