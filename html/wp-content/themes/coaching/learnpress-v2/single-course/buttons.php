...
$purchase_button_text     = apply_filters( 'learn_press_purchase_button_text', __( 'Buy this course', 'coaching' ) );
$enroll_button_text       = apply_filters( 'learn_press_enroll_button_text', __( 'Take this course', 'coaching' ) );
$retake_button_text       = apply_filters( 'learn_press_retake_button_text', __( 'Retake', 'coaching' ) );
...
if ( $user->can( 'enroll-course', $course->id ) === 'enough' ) {
	learn_press_display_message( apply_filters( 'learn_press_course enough students_notice', __( 'The class is full so the enrollment is close. Please contact the site admin.', 'coaching' ) ) );
	return;
}
?>
...
<?php else: ?>
	...
	<?php } elseif ( in_array( $order_status, array( 'lp-processing', 'lp-on-hold' ) ) ) { ?>
		<?php learn_press_display_message( '<p>' . apply_filters( 'learn_press_user_course_pending_message', __( 'You have purchased this course. Please wait for approval.', 'coaching' ), $course, $user ) . '</p>' ); ?>
	<?php } elseif ( $order_status && $order_status != 'lp-completed' ) { ?>
		<?php learn_press_display_message( '<p>' . apply_filters( 'learn_press_user_can_not_purchase_course_message', __( 'Sorry, you can not purchase this course', 'coaching' ), $course, $user ) . '</p>' ); ?>
	<?php } ?>
<?php endif; ?>
