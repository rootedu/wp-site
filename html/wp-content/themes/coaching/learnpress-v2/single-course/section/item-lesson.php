...
<li <?php learn_press_course_item_class( $item->ID ); ?> data-type="<?php echo $item->post_type; ?>">
	...
	<div class="meta-left">
		...
		<div class="index"><?php echo '<span class="label">' . esc_html__( 'Lecture', 'coaching' ) . '</span> ' . $index; ?></div>
	</div>
	...
	<?php if ( $item_preview ): ?>
		<a class="lesson-preview button-load-item" href="<?php echo $course->get_item_link( $item->ID ); ?>" data-id="<?php echo $item->ID; ?>" data-complete-nonce="<?php echo wp_create_nonce( 'learn-press-complete-' . $item->post_type . '-' . $item->ID ); ?>"><?php esc_html_e( 'Preview', 'coaching' ); ?></a>
	<?php endif; ?>

	<?php
	//if ( !$viewable ) {
	//echo '<div class="locked"><i class="fa fa-fa-lock"></i>' . esc_html__( 'Locked', 'coaching' ) . '</div>';
