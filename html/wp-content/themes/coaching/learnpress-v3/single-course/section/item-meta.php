...
<?php if ( $item->is_preview() ) { ?>
    <a title="<?php esc_html_e( 'Previews', 'coaching' ); ?>" class="lesson-preview button-load-item" href="<?php echo $item->get_permalink(); ?>" data-id="<?php echo $item->get_id(); ?>" data-complete-nonce="<?php echo wp_create_nonce( 'learn-press-complete-' . $item->get_item_type() . '-' . $item->get_id() ); ?>">
        <i class="fa fa-eye"
           data-preview="<?php esc_html_e( 'Preview', 'coaching' ); ?>"></i>
    </a>
<?php } else { ?>
