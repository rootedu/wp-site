...
<div id="lp-single-course" class="lp-single-course">
	...
	<?php if ( ! learn_press_get_page_link( 'checkout' ) && ( $user->is_admin() || $user->is_instructor() ) ) { ?>
		...
		<?php $message = __( 'LearnPress <strong>Checkout</strong> page is not set up. ', 'coaching' );

		if ( $user->is_instructor() ) {
			$message .= __( 'Please contact to administrator for setting up this page.', 'coaching' );
		} else {
			$message .= sprintf( __( 'Please <a href="%s" target="_blank">setup</a> it so user can purchase a course.', 'coaching' ), admin_url( 'admin.php?page=learn-press-settings&tab=checkout' ) );
		} ?>
