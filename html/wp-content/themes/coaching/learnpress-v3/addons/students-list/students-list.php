...
<?php if ( $course ) { ?>
    ...
    <div class="course-students-list">
		...
		<?php if ( $students = $curd->get_user_enrolled( $course->get_ID(), $limit ) ) { ?>
			<?php
            $students_list_heading     = apply_filters( 'learn_press_students_list_heading', __( 'Students Enrolled', 'coaching' ) );
            ...
            <div class="filter-students">
                <label for="students-list-filter"><?php echo esc_html__( 'Filter', 'coaching' ); ?></label>
                <select id="students-list-filter">
                    <?php
                    $filters = array(
                        'all'         => esc_html__( 'All', 'coaching' ),
                        'in-progress' => esc_html__( 'In Progress', 'coaching' ),
                        'finished'    => esc_html__( 'Finished', 'coaching' )
                    );
                        ...
                        <div class="user-info">
                            ...
                            <div class="right-info">
                                ...
                                <div class="course-progress">
                                    <span class="course-result"><?php echo esc_html__( 'Learning', 'coaching' );?></span><?php echo $result; ?>%
                                </div>
                            </div>
                        </div>
                    </li>
			...
			if ( $other_student && $limit == - 1 ) {
				echo '<p class="additional-students">and ' . sprintf( _n( 'one student enrolled.', '%s students enrolled.', $other_student, 'coaching' ), $other_student ) . '</p>';
			}
			?>
		<?php } else { ?>
            <div class="students empty">
				<?php if ( $course->students ) {
					echo apply_filters( 'learn_press_course_count_student', sprintf( _n( 'One student enrolled.', '%s students enrolled.', $course->students, 'coaching' ), $course->students ) );
				} else {
					echo apply_filters( 'learn_press_course_no_student', __( 'No student enrolled.', 'coaching' ) );
				} ?>
            </div>
		<?php } ?>
    </div>
	...
	echo __( 'Course ID invalid, please check it again.', 'coaching' );
}