...
if ( !function_exists( 'thim_learnpress_breadcrumb' ) ) {
	function thim_learnpress_breadcrumb() {
		...
		// Home page
		echo '<li itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem"><a itemprop="item" href="' . esc_html( get_home_url() ) . '" title="' . esc_attr__( 'Home', 'coaching' ) . '"><span itemprop="name">' . esc_html__( 'Home', 'coaching' ) . '</span></a></li>';
					...
					<?php foreach ( $related_courses as $course_item ) : ?>
						...
						<article class="course-grid-3 lpr_course">
							<div class="course-item">
								...
								echo '<a href="' . esc_url( get_the_permalink( $course_item->ID ) ) . '" >';
								...
								echo '<a class="course-readmore" href="' . esc_url( get_the_permalink( $course_item->ID ) ) . '">' . esc_html__( 'Read More', 'coaching' ) . '</a>';
								echo '</div>';
								?>
	...
	function thim_courses_collection_breadcrumb() {
		...
		if ( is_single() ) {
			if ( get_post_type() == 'lp_collection' ) {
				echo '<li itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem"><a itemprop="item" href="' . esc_url( get_post_type_archive_link( 'lp_collection' ) ) . '" title="' . esc_attr__( 'Collections', 'coaching' ) . '"><span itemprop="name">' . esc_html__( 'Collections', 'coaching' ) . '</span></a></li>';
			}
			echo '<li itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem"><span itemprop="name" title="' . esc_attr( get_the_title() ) . '">' . esc_html( get_the_title() ) . '</span></li>';
		} else {
			echo '<li itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem"><span itemprop="name">' . esc_html__( 'Collections', 'coaching' ) . '</span></li>';
		}
	...
	if ( !function_exists( 'thim_update_certificates_tab' ) ) {
		function thim_update_certificates_tab( $tabs, $user ) {
			...
			$tabs[$certificate->get_tab_slug()] = array(
				'title'    => '<i class="fa fa-rebel"></i>' . esc_html__( 'Certificates', 'coaching' ),
	...
	function thim_redirect_url_lp_enroll() {
		if ( ! is_user_logged_in() && empty( $_REQUEST['checkout'] ) ) {
			...
			if ( $user->has_finished_course( $post->ID ) ) {
				learn_press_add_message( __( 'You have already finished course', 'coaching' ) );
				$redirect = true;
			} elseif ( $user->has_purchased_course( $post->ID ) ) {
				learn_press_add_message( __( 'You have already enrolled in this course', 'coaching' ) );
				$redirect = true;
			}
			if ( $redirect ) {
