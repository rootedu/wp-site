...
if ( !function_exists( 'thim_paging_nav' ) ) :
	...
	function thim_paging_nav() {
		...
		$links = paginate_links( array(
			...
			'prev_text' => esc_html__( '<', 'coaching' ),
			'next_text' => esc_html__( '>', 'coaching' ),
			'type'      => 'list'
		) );
...
if ( !function_exists( 'thim_post_nav' ) ) :
	...
	function thim_post_nav() {
		...
		$pre_id = get_previous_post()->ID;
		if ( isset( $pre_id ) && get_the_post_thumbnail( get_previous_post()->ID, 'thumbnail' ) ) {
			$prev_img = '<span class="img">' . get_the_post_thumbnail( get_previous_post()->ID, 'thumbnail' ) . '</span>';
		} else {
			$prev_img = "";
		}

		if ( isset( $pre_id ) && get_the_post_thumbnail( get_next_post()->ID, 'thumbnail' ) ) {
			...
			if ( isset( $theme_options_data['thim_show_comment'] ) && $theme_options_data['thim_show_comment'] == 1 ) {
				...
				<?php if ( !post_password_required() && ( comments_open() || '0' != get_comments_number() ) ) :
					...
					<li class="comment-total">
						<?php comments_popup_link( esc_html__( '0 comment', 'coaching' ), esc_html__( '1 comment', 'coaching' ), esc_html__( '% comments', 'coaching' ) ); ?>
					</li>
					<?php
				endif;
			}
...
if ( !function_exists( 'thim_entry_footer' ) ) :
	...
	function thim_entry_footer() {
		...
		if ( 'post' == get_post_type() ) {
			...
			$categories_list = get_the_category_list( esc_html__( ', ', 'coaching' ) );
			if ( $categories_list && thim_categorized_blog() ) {
				printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'coaching' ) . '</span>', $categories_list );
			}
			/* translators: used between list items, there is a space after the comma */
			$tags_list = get_the_tag_list( '', esc_html__( ', ', 'coaching' ) );
			if ( $tags_list ) {
				printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'coaching' ) . '</span>', $tags_list );
			}
		}

		if ( !is_single() && !post_password_required() && ( comments_open() || get_comments_number() ) ) {
			echo '<span class="comments-link">';
			comments_popup_link( esc_html__( 'Leave a comment', 'coaching' ), esc_html__( '1 comment', 'coaching' ), esc_html__( '% comments', 'coaching' ) );
			echo '</span>';
		}

		edit_post_link( esc_html__( 'Edit', 'coaching' ), '<span class="edit-link">', '</span>' );
	}
...
if ( !function_exists( 'thim_archive_title' ) ) :
	...
	function thim_archive_title( $before = '', $after = '' ) {
		if ( is_category() ) {
			$title = sprintf( esc_html__( '%s', 'coaching' ), single_cat_title( '', false ) );
		} elseif ( is_tag() ) {
			$title = sprintf( esc_html__( '%s', 'coaching' ), single_tag_title( '', false ) );
		} elseif ( is_author() ) {
			$title = sprintf( esc_html__( '%s', 'coaching' ), '<span class="vcard">' . get_the_author() . '</span>' );
		} elseif ( is_year() ) {
			$title = sprintf( esc_html__( 'Year: %s', 'coaching' ), get_the_date( _x( 'Y', 'yearly archives date format', 'coaching' ) ) );
		} elseif ( is_month() ) {
			$title = sprintf( esc_html__( 'Month: %s', 'coaching' ), get_the_date( _x( 'F Y', 'monthly archives date format', 'coaching' ) ) );
		} elseif ( is_day() ) {
			$title = sprintf( esc_html__( 'Day: %s', 'coaching' ), get_the_date( _x( 'F j, Y', 'daily archives date format', 'coaching' ) ) );
		} elseif ( is_tax( 'post_format', 'post-format-aside' ) ) {
			$title = _x( 'Asides', 'post format archive title', 'coaching' );
		} elseif ( is_tax( 'post_format', 'post-format-gallery' ) ) {
			$title = _x( 'Galleries', 'post format archive title', 'coaching' );
		} elseif ( is_tax( 'post_format', 'post-format-image' ) ) {
			$title = _x( 'Images', 'post format archive title', 'coaching' );
		} elseif ( is_tax( 'post_format', 'post-format-video' ) ) {
			$title = _x( 'Videos', 'post format archive title', 'coaching' );
		} elseif ( is_tax( 'post_format', 'post-format-quote' ) ) {
			$title = _x( 'Quotes', 'post format archive title', 'coaching' );
		} elseif ( is_tax( 'post_format', 'post-format-link' ) ) {
			$title = _x( 'Links', 'post format archive title', 'coaching' );
		} elseif ( is_tax( 'post_format', 'post-format-status' ) ) {
			$title = _x( 'Statuses', 'post format archive title', 'coaching' );
		} elseif ( is_tax( 'post_format', 'post-format-audio' ) ) {
			$title = _x( 'Audio', 'post format archive title', 'coaching' );
		} elseif ( is_tax( 'post_format', 'post-format-chat' ) ) {
			$title = _x( 'Chats', 'post format archive title', 'coaching' );
		} elseif ( is_post_type_archive() ) {
			$title = sprintf( esc_html__( '%s', 'coaching' ), post_type_archive_title( '', false ) );
		} elseif ( is_tax() ) {
			$tax = get_taxonomy( get_queried_object()->taxonomy );
			/* translators: 1: Taxonomy singular name, 2: Current taxonomy term */
			$title = sprintf( esc_html__( '%1$s: %2$s', 'coaching' ), $tax->labels->singular_name, single_term_title( '', false ) );
		} elseif ( is_404() ) {
			$title = esc_html__( '404 Page', 'coaching' );
		} elseif ( is_search() ) {
			$title = sprintf( esc_html__( 'Search Results for: %s', 'coaching' ), get_search_query() );
		} else {
			$title = esc_html__( 'Archives', 'coaching' );
		}
		...
		if ( function_exists( 'is_bbpress' ) ) {
			if ( bbp_is_search() ) {
				...
				if ( current_user_can( 'manage_topic_tags' ) ) {
					$tag_data[] = '' . esc_html__( '(Edit)', 'coaching' ) . '';
				}

				// Implode the results of the tag data
				$pre_current_text = sprintf( esc_html__( 'Topic Tag: %s', 'coaching' ), implode( ' ', $tag_data ) );

				// Edit Topic Tag
			} elseif ( bbp_is_topic_tag_edit() ) {
				$pre_current_text = esc_html__( 'Edit', 'coaching' );
			} else {
