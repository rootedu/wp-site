						...
						<div class="lp_pmpro_price_level">
							<div class="price-wrap">
								<?php if ( pmpro_isLevelFree( $level ) ): ?>
									...
									echo '<p class="price">' . esc_html( 'Free', 'coaching' ) . '</p>';

									if ( $level->expiration_number ) {
										echo '<p class="expired">' . sprintf( __( "expires after %d %s.", "coaching" ), $level->expiration_number, pmpro_translate_billing_period( $level->expiration_period, $level->expiration_number ) ) . '</p>';
									}
					...
					<footer>
						<div class="button">
							<?php if ( empty( $current_user->membership_level->ID ) || ! $current_level ) { ?>
								<a class="pmpro_btn pmpro_btn-select" href="<?php echo pmpro_url( 'checkout', '?level=' . $level->id, 'https' ) ?>"><?php _e( 'GET IT NOW', 'coaching' ); ?></a>
...
<nav id="nav-below" class="navigation" role="navigation">
	<div class="nav-previous alignleft">
		<?php if(!empty($current_user->membership_level->ID)) { ?>
			<a href="<?php echo pmpro_url("account")?>"><?php esc_html_e('&larr; Return to Your Account', 'coaching');?></a>
		<?php } else { ?>
			<a href="<?php echo home_url()?>"><?php _e('&larr; Return to Home', 'coaching');?></a>
		<?php } ?>
	</div>
</nav>
