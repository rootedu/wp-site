...
<div id="pmpro_level-<?php echo $pmpro_level->id; ?>">
	<form id="pmpro_form" class="pmpro_form" action="<?php if ( !empty( $_REQUEST['review'] ) ) echo pmpro_url( "checkout", "?level=" . $pmpro_level->id ); ?>" method="post">
		...
		<?php if ( $pmpro_review ) { ?>
			<p><?php _e( 'Almost done. Review the membership information and pricing below then <strong>click the "Complete Payment" button</strong> to finish your order.', 'coaching' ); ?></p>
		<?php } ?>
		...
		<table id="pmpro_pricing_fields" class="pmpro_checkout" width="100%" cellpadding="0" cellspacing="0" border="0">
			...
			<tr>
				<th>
					<h3 class="pmpro_thead-name"><?php _e( 'Membership Level', 'coaching' ); ?></h3>
					<?php if ( count( $pmpro_levels ) > 1 ) { ?><span class="pmpro_thead-msg">
						<a href="<?php echo pmpro_url( "levels" ); ?>"><?php _e( 'Change level', 'coaching' ); ?></a>
						</span><?php } ?>
				</th>
			</tr>
			</thead>
			...
			<tr>
				<td class="lp-pmpro-td">
					...
					<div class="lp-pmpro-content">
						<?php echo '<div class="lp-pmpro-desc"><label>' . esc_html__( 'Membership level:', 'coaching' ) . '</label>' . $pmpro_level->name . '</div>'; ?>

						<?php if ( $discount_code && pmpro_checkDiscountCode( $discount_code ) ) { ?>
							<?php printf( __( '<p class="pmpro_level_discount_applied">The <strong>%s</strong> code has been applied to your order.</p>', 'coaching' ), $discount_code ); ?>
						<?php } ?>
						<?php echo '<div class="lp-pmpro-desc lp-pmpro-price"><label>' . esc_html__( 'Price:', 'coaching' ) . '</label>' . pmpro_getLevelCost( $pmpro_level, true, true ) . '</div>'; ?>

						<?php echo '<div class="lp-pmpro-desc"><label>' . esc_html__( 'Expires after:', 'coaching' ) . '</label>' . sprintf( __( "%d %s", "coaching" ), $pmpro_level->expiration_number, pmpro_translate_billing_period( $pmpro_level->expiration_period, $pmpro_level->expiration_number ) ) . '</div>'; ?>
						...
						<?php if ( $pmpro_show_discount_code ) { ?>
							...
							<?php if ( $discount_code && !$pmpro_review ) { ?>
								<p id="other_discount_code_p" class="pmpro_small">
									<a id="other_discount_code_a" href="#discount_code"><?php _e( 'Click here to change your discount code', 'coaching' ); ?></a>.
								</p>
							<?php } elseif ( !$pmpro_review ) { ?>
								<p id="other_discount_code_p" class="pmpro_small"><?php _e( 'Do you have a discount code?', 'coaching' ); ?>
									<a id="other_discount_code_a" href="#discount_code"><?php _e( 'Click here to enter your discount code', 'coaching' ); ?></a>.
								</p>
							<?php } elseif ( $pmpro_review && $discount_code ) { ?>
								<p>
									<strong><?php _e( 'Discount Code', 'coaching' ); ?>:</strong> <?php echo $discount_code ?>
								</p>
							<?php } ?>
						...
						if ( !empty( $level_description ) )
							echo '<div class="lp-pmpro-desc lp-pmpro-description"><label>' . esc_html__( 'Description:', 'coaching' ) . '</label>' . $pmpro_level->description . '</div>';
						?>
					</div>
				</td>
			</tr>
			<?php if ( $pmpro_show_discount_code ) { ?>
				<tr id="other_discount_code_tr" style="display: none;">
					<td>
						<div>
							<label for="other_discount_code"><?php _e( 'Discount Code', 'coaching' ); ?></label>
							...
							<input type="button" name="other_discount_code_button" id="other_discount_code_button" value="<?php _e( 'Apply', 'coaching' ); ?>" />
						</div>
					</td>
				</tr>
			<?php } ?>
		...
		<?php if ( !$skip_account_fields && !$pmpro_review ) { ?>
			<table id="pmpro_user_fields" class="pmpro_checkout" width="100%" cellpadding="0" cellspacing="0" border="0">
				...
				<tr>
					<th>
						<h3 class="pmpro_thead-name"><?php _e( 'Account Information', 'coaching' ); ?></h3>
						<span class="pmpro_thead-msg"><?php _e( 'Already have an account?', 'coaching' ); ?>
							<a href="<?php echo wp_login_url( pmpro_url( "checkout", "?level=" . $pmpro_level->id ) ) ?>"><?php _e( 'Login here', 'coaching' ); ?></a></span>
					</th>
				</tr>
				</thead>
				<tbody>
				<tr class="lp-pmpro-account-info">
					<td class="lp-pmpro-td">
						<div class="lp-pmpro-desc">
							<label for="username"><?php _e( 'Username', 'coaching' ); ?></label>
						...
						<div class="lp-pmpro-desc">
							<label for="password"><?php _e( 'Password', 'coaching' ); ?></label>
						...
						if ( $pmpro_checkout_confirm_password ) {
							?>
							<div class="lp-pmpro-desc">
								<label for="password2"><?php _e( 'Confirm Password', 'coaching' ); ?></label>
						...
						<div class="lp-pmpro-desc">
							<label for="bemail"><?php _e( 'E-mail Address', 'coaching' ); ?></label>
						...
						if ( $pmpro_checkout_confirm_email ) {
							?>
							<div class="lp-pmpro-desc">
								<label for="bconfirmemail"><?php _e( 'Confirm E-mail Address', 'coaching' ); ?></label>
		...
		<?php } elseif ( $current_user->ID && !$pmpro_review ) { ?>
			...
			<p id="pmpro_account_loggedin" class="message message-notice">
				<?php printf( __( 'You are logged in as <strong>%s</strong>. If you would like to use a different account for this membership, <a href="%s">log out now</a>.', 'coaching' ), $current_user->user_login, wp_logout_url( $_SERVER['REQUEST_URI'] ) ); ?>
			</p>
		<?php } ?>
		...
		<?php if ( pmpro_getGateway() == "paypal" && empty( $pmpro_review ) ) { ?>
			<table id="pmpro_payment_method" class="pmpro_checkout top1em" width="100%" cellpadding="0" cellspacing="0" border="0" <?php if ( !$pmpro_requirebilling ) { ?>style="display: none;"<?php } ?>>
				<thead>
				<tr>
					<th><?php _e( 'Choose your Payment Method', 'coaching' ); ?></th>
				</tr>
				</thead>
				<tbody>
				<tr>
					<td>
						...
						<span class="gateway_paypal">
							...
							<a href="javascript:void(0);" class="pmpro_radio"><?php _e( 'Check Out with a Credit Card Here', 'coaching' ); ?></a>
						</span>
						<span class="gateway_paypalexpress">
							...
							<a href="javascript:void(0);" class="pmpro_radio"><?php _e( 'Check Out with PayPal', 'coaching' ); ?></a>
						</span>
						</div>
					</td>
				</tr>
...
ffff
		...
		if ( $pmpro_include_billing_address_fields ) {
			...
			<table id="pmpro_billing_address_fields" class="pmpro_checkout top1em" width="100%" cellpadding="0" cellspacing="0" border="0" <?php if ( !$pmpro_requirebilling || apply_filters( "pmpro_hide_billing_address_fields", false ) ){ ?>style="display: none;"<?php } ?>>
				<thead>
				<tr>
					<th>fffffffffff<?php _e( 'Billing Address', 'coaching' ); ?></th>
				</tr>
				</thead>
				<tbody>
				<tr>
					<td>
						<div>
							<label for="bfirstname"><?php _e( 'First Name', 'coaching' ); ?></label>
						...
						<div>
							<label for="blastname"><?php _e( 'Last Name', 'coaching' ); ?></label>
						...
						<div>
							<label for="baddress1"><?php _e( 'Address 1', 'coaching' ); ?></label>
						...
						<div>
							<label for="baddress2"><?php _e( 'Address 2', 'coaching' ); ?></label>
						...
						if ( $longform_address ) {
							?>
							<div>
								<label for="bcity"><?php _e( 'City', 'coaching' ); ?></label>
							...
							<div>
								<label for="bstate"><?php _e( 'State', 'coaching' ); ?></label>
							...
							<div>
								<label for="bzipcode"><?php _e( 'Postal Code', 'coaching' ); ?></label>
							...
							?>
							<div>
								<label for="bcity_state_zip"><?php _e( 'City, State Zip', 'coaching' ); ?></label>
						...
						if ( $show_country ) {
							?>
							<div>
								<label for="bcountry"><?php _e( 'Country', 'coaching' ); ?></label>
						...
						?>
						<div>
							<label for="bphone"><?php _e( 'Phone', 'coaching' ); ?></label>
						...
						<?php if ( $skip_account_fields ) { ?>
							...
							if ( $pmpro_checkout_confirm_email ) {
								?>
								<div>
									<label for="bconfirmemail"><?php _e( 'Confirm E-mail', 'coaching' ); ?></label>
									<input id="bconfirmemail" name="bconfirmemail" type="<?php echo( $pmpro_email_field_type ? 'email' : 'text' ); ?>" class="input <?php echo pmpro_getClassForField( "bconfirmemail" ); ?>" size="30" value="<?php echo esc_attr( $bconfirmemail ) ?>" />
		...
		if ( $pmpro_include_payment_information_fields ) {
			...
			<table id="pmpro_payment_information_fields" class="pmpro_checkout top1em" width="100%" cellpadding="0" cellspacing="0" border="0" <?php if ( !$pmpro_requirebilling || apply_filters( "pmpro_hide_payment_information_fields", false ) ) { ?>style="display: none;"<?php } ?>>
				...
				<tr>
					<th>
						<span class="pmpro_thead-name"><?php _e( 'Payment Information', 'coaching' ); ?></span>
						<span class="pmpro_thead-msg"><?php printf( __( 'We Accept %s', 'coaching' ), $pmpro_accepted_credit_cards_string ); ?></span>
					</th>
				</tr>
				</thead>
				<tbody>
					...
					<td>
						...
						if ( $pmpro_include_cardtype_field ) {
							...
							<div class="pmpro_payment-card-type">
								<label for="CardType"><?php _e( 'Card Type', 'coaching' ); ?></label>
						...
						<div class="pmpro_payment-account-number">
							<label for="AccountNumber"><?php _e( 'Card Number', 'coaching' ); ?></label>
						...
						<div class="pmpro_payment-expiration">
							<label for="ExpirationMonth"><?php _e( 'Expiration Date', 'coaching' ); ?></label>
						...
						if ( $pmpro_show_cvv ) { ?>
							<div class="pmpro_payment-cvv">
								<label for="CVV"><?php _e( 'CVV', 'coaching' ); ?></label>
								...
								<small>(<a href="javascript:void(0);" onclick="javascript:window.open('<?php echo pmpro_https_filter( PMPRO_URL ) ?>/pages/popup-cvv.html','cvv','toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=yes, resizable=yes, width=600, height=475');"><?php _e( "what's this?", 'coaching' ); ?></a>)
								</small>
							</div>
						<?php } ?>
						...
						<?php if ( $pmpro_show_discount_code ) { ?>
							<div class="pmpro_payment-discount-code">
								<label for="discount_code"><?php _e( 'Discount Code', 'coaching' ); ?></label>
								...
								<input type="button" id="discount_code_button" name="discount_code_button" value="<?php _e( 'Apply', 'coaching' ); ?>" />
			...
			<table id="pmpro_tos_fields" class="pmpro_checkout top1em" width="100%" cellpadding="0" cellspacing="0" border="0">
				...
				<tr class="odd">
					<td>
						...
						<label class="pmpro_normal pmpro_clickable" for="tos"><?php printf( __( 'I agree to the %s', 'coaching' ), $tospage->post_title ); ?></label>
					</td>
				</tr>
				</tbody>
			</table>
		...
		<div class="pmpro_submit">
			<?php if ( $pmpro_review ) { ?>
				...
				<input type="submit" class="pmpro_btn pmpro_btn-submit-checkout" value="<?php _e( 'Complete Payment', 'coaching' ); ?> &raquo;" />
			</span>
			...
			<?php } else { ?>
				...
				if ( $pmpro_checkout_default_submit_button ) {
					...
					<input type="submit" class="pmpro_btn pmpro_btn-submit-checkout" value="<?php if ( $pmpro_requirebilling ) {
						_e( 'Submit and Check Out', 'coaching' );
					} else {
						_e( 'Submit and Confirm', 'coaching' );
					} ?> &raquo;" />
				</span>
					<?php
				}
			...
			<?php
			$processing_message = apply_filters( "pmpro_processing_message", __( "Processing...", "coaching" ) );
