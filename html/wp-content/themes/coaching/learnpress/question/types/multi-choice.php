...
?>
<div <?php learn_press_question_class( $this, $args ); ?> data-id="<?php echo esc_attr( $this->id ); ?>" data-type="multi-choice">
	...
	<ul class="learn-press-question-options">
		<?php if ( $answers = $this->answers ) foreach ( $answers as $k => $answer ): ?>
			...
			<li<?php echo ( $answer_class ) ? ' class="' . join( ' ', $answer_class ) . '"' : ''; ?>>
				...
				<label>
					<input type="checkbox" name="learn-press-question-<?php echo esc_attr( $this->id ); ?>[]" <?php checked( $this->is_selected_option( $answer, $answered ) ); ?> value="<?php echo esc_attr( $answer['value'] ); ?>" <?php echo ( $checked ) ? 'disabled="disabled"' : ''; ?> />
